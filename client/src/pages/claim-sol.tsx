...
export default function SolRefund() {
  ...
  // Fetch wallet balance for low balance warning
  const balanceQuery = useQuery({
    queryKey: ['/api/wallet/balance', publicKey?.toString()],
    queryFn: async () => {
  ...
  const burnTokenMutation = useMutation({
    mutationFn: async (tokenMint: string) => {
      ...
      if (!response.ok) {
        ...
        if (errorData.error === "Insufficient SOL for transaction fees") {
          throw new Error(errorData.details?.message || "Insufficient SOL for transaction fees");
        }
...
{activeTab === 'referrals' ? 'Earn 35% commission from your referrals — just by helping others!' : activeTab === 'burnTokens' ? 'Burn Unwanted Tokens.' : 'Get your SOL back!'}
          ...
          {/* Low Balance Warning */}
          {isConnected && balanceQuery.data?.isLowBalance && (
