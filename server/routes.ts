...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/sol-refund/prepare-transaction", async (req, res) => {
    try {
      ...
      // NOTE: Fee transfers removed from main transaction to avoid "insufficient SOL" errors
      // Fees will be collected in a separate transaction after SOL recovery
      // This prevents wallet simulation from failing on fee transfers before SOL recovery
      
      console.log(`Transaction will recover ${totalSolReclaimed} SOL, fees (${totalFeeAmount} SOL) will be collected separately`);
      console.log(`Platform fee: ${platformFeeAmount} SOL, Referral fee: ${referralFeeAmount} SOL`);
