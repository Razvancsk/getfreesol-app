...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/sol-refund/prepare-transaction", async (req, res) => {
    try {
      ...
      // Add fee transfers AFTER account closures (SOL recovered first, then fees paid)
      const { SystemProgram } = await import('@solana/web3.js');
      
      console.log(`Transaction order: 1) Recover ${totalSolReclaimed} SOL, 2) Pay ${totalFeeAmount} SOL in fees`);
      ...
      if (platformFeeAmount > 0) {
        ...
        console.log(`✅ Added platform fee transfer: ${platformFeeAmount} SOL`);
      }
      ...
      if (referralFeeAmount > 0 && referralCodeData) {
        ...
        console.log(`✅ Added referral fee transfer: ${referralFeeAmount} SOL`);
      }
