...
- **Crates & Levels System**: Mini-game where users earn levels from their points and open tiered crates every 24 hours to win SOL. Level formula: `min(100, floor(points/500) + 1)` — 500 points per level. 10 crate tiers (Genesis→Apex) unlock every 10 levels. SOL rewards are sent from RELAYER_PRIVATE_KEY wallet. Discrete probability tiers per crate. Routes: `GET /api/crates/status/:wallet`, `POST /api/crates/open`, `GET /api/crates/history/:wallet`. Data in `crate_opens` table.
