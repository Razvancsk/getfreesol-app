...
- **Developer API Platform**: Jupiter-style developer platform with platform-managed referral wallet system. Developers create referral accounts via wallet signature, receiving a newly generated Solana wallet address (encrypted keypair stored server-side with AES-256-GCM) for secure fee collection. During account creation, developers deposit 0.001 SOL to initialize the referral wallet, ensuring it can receive fees without "Insufficient Funds For Rent" errors. Supports multiple token mints (SOL, USDC, USDT, BONK, JUP, etc.) with separate token account tracking. Developers earn 80% of collected fees. API documentation is publicly accessible.
